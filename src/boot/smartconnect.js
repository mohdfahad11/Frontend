...
export const TransactionOutcome = Object.freeze({
...
// ======================================================
export function sendParingRequest(pairingCode) {
  ...
  return new Promise((resolve, reject) => {
    ...
    axios.put(pairingEndpoint, null,
      { params: parameters},
...
// ======================================================
export function createTransaction(amount, transactionType) {
  ...
  return new Promise((resolve, reject) => {
    ...
    axios.post(transactionEndpoint, null,
      { params: parameters},
      {headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',}}
    )
    .then((response) => {
      ...
      if (response.status == 200) {
        ...
        if (response && response.data && response.data.data.PollingUrl) {

          // return the polling URL
          resolve(response.data.data.PollingUrl);
          return;
...
//     - reject(string) - the string will contain the error message
// =====================================================
export function pollForOutcome(pollingUrl, delayed) {
    ...
    .then((response) => {
      ...
      if(response.status == 200) {
        if (response && response.data) {
          ...
          let transactionResult = response.data.data.TransactionResult;
